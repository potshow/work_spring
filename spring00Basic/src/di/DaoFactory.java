...
public class DaoFactory {
	...
	// 1. DaoFactory 클래스가 메모리에 올라가게 되면 실행되는 static 목록
	static {
		...
		try {
			...
			// 7. 자식 앨리머트(노드)들을 순회한다.
			for (int i = 0; i < list.getLength(); i++) {
				// 8. 만약 해당 아이템이 앨리먼트일 경우
				if (list.item(i) instanceof Element) {
					...
					// 12. daos Map 에 id 값과 className값을 저장한다.
	...
	/*
	 * getDao는 daos 에 저장된 dao 클래스 이름을 가져온 뒤
	 * 해당 클래스의 객체를 만들어서 리턴해주는 메서드
	 */
	public static UserDao getDao(String type) {
		...
		try {
			...
			/*
			 * 3. 가져온 클래스 정보 객체에 있는 newInstance 메서드를 이용하여
			 * 해당 클래스의 객체를 생성한다.
			 */
